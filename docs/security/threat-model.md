...
| TM-12 | Secret theft from configuration or image | Production requires the complete database DSN and sensitive values from files, rejects alternate inline password syntax, uses a scratch non-root image, read-only root, workload identity, and no Secret manifest | Host, node, or platform secret-controller compromise is outside the process boundary |
